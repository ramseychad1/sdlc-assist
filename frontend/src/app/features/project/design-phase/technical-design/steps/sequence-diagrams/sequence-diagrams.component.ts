...
const ARTIFACT_STEP_STYLES = `
  ...
  .btn.btn-success { background: #16a34a; color: white; border-color: #16a34a; opacity: 0.8; }
  .btn.btn-ghost { border-color: transparent; }
`;

@Component({
  selector: 'app-sequence-diagrams',
  standalone: true,
        ...
        <div class="context-row">
          <div class="context-card">
            ...
            <div class="context-card-value">
              @if (project()?.apiContractContent) {
                <span class="ctx-available">API Contract generated</span>
              } @else {
                <span class="ctx-missing">Generate API Contract first</span>
              }
            </div>
          </div>
          <div class="context-card">
      ...
      @if (pageState() === 'generating') {
        <div class="generating-card">
          <div class="gen-header">
            ...
            <span>Generating Sequence Diagrams...</span>
          </div>
      ...
      @if (pageState() === 'result') {
        <div class="result-card">
          <div class="result-header">
            <div class="result-title">
              <lucide-icon name="git-branch" [size]="16"></lucide-icon>
              Sequence Diagrams
              <span class="result-date">Generated {{ project()?.sequenceDiagramsGeneratedAt | date:'MMM d, yyyy' }}</span>
            </div>
          ...
          <div class="artifact-content">
            <markdown [data]="project()!.sequenceDiagramsContent!" mermaid></markdown>
          </div>
        </div>
        ...
        <div class="action-bar">
          ...
          @if (techDesignComplete()) {
            ...
            <button class="btn btn-primary" (click)="completeTechDesign()" [disabled]="completing()">
              @if (completing()) {
...
export class SequenceDiagramsComponent implements OnInit {
  ...
  canGenerate = computed(() => !!this.project()?.apiContractContent);
  techDesignComplete = computed(() => this.project()?.techDesignStatus === 'COMPLETE');
  ...
  ngOnInit(): void {
    ...
    if (id) {
      this.projectService.getById(id).subscribe(proj => {
        ...
        if (proj.sequenceDiagramsContent) this.pageState.set('result');
      });
    }
  }
  ...
  startGeneration(): void {
    ...
    this.progress.set(0);
    this.techDesignService.generateArtifact(id, 'sequence-diagrams').subscribe({
      next: event => {
  ...
  regenerate(): void {
    const id = this.projectId;
    if (!id) return;
    this.projectService.clearArtifact(id, 'sequence-diagrams').subscribe(proj => {
      this.project.set(proj);
  ...
  completeTechDesign(): void {
    const id = this.projectId;
    if (!id || this.completing()) return;
    this.completing.set(true);
    this.projectService.completePhase(id, 'TECHNICAL_DESIGN').subscribe({
      next: proj => {
        this.project.set(proj);
